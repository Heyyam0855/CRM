...
### Yeni Feature Üçün Tövsiyə Olunan Prosess

```
1. @backend-architect   → Model + Service dizaynı
        ↓
2. @db-engineer         → Schema optimallaşdırması + indekslər
        ↓
3. @backend-architect   → CBV views + URL-lər
        ↓
4. @integrations-dev    → Xarici API inteqrasiyaları (lazım olsa)
        ↓
5. @payments-dev        → Ödəniş məntiqi (lazım olsa)
        ↓
6. @frontend-dev        → Django Templates + HTMX
        ↓
7. @notifications-dev   → Bildirişlər + Celery task-lar
        ↓
8. @security-expert     → Security audit
        ↓
9. @testing-specialist  → Test suite yaz
        ↓
10. @devops-engineer    → Deployment konfiqurasiyası
```
